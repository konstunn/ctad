...
@
	...
	\item Исследовать сходимость к предельным распределениям, для чего
		...
		распределение статистки выбора различных объемов выборок $m_i \ne n_i$.
...
df <- NULL
for (n in c(10, 20, 50, 100, 200, 400)) {
...
% TODO: plot pkolm
<<plot, fig.cap=caption>>=
caption <- 'Эмпирическая функция распределения статистики критерия однородности
Смирнова, объем выборки $n = 100$.'

library(kolmim)

#dft <- data.frame(subset(df, n==100)$x)

#dft$y <- sapply(subset(df, n==100)$x, function(x) pkolm(x, 50))

#df[which(df$n=='100'),]$x <- subset(df, n=='100')$x * 5

ggplot(data=df, mapping=aes(x=x, colour=n)) +
	stat_ecdf()
@
