...
caption <- 'Эмпирические функции распределения статистики критерия при
различных объемах выборки $n$ и предельный закон $\\chi_2^2$.'

nn <- 10000
dfp100 <- head(df100, nn)
dfp400  <- head(df400, nn)
dfp200 <- head(df200, nn)
dfp800 <- head(df800, nn)
dfp1600 <- head(df1600, nn)
dfp3200 <- head(df3200, nn)
dfp6400 <- head(df6400, nn)

dfp <- rbind(dfp100, dfp200, dfp800, dfp6400)

ggplot(data=dfp, size=.5, mapping=aes(x=x, linetype=n)) +
	stat_ecdf() +
	stat_function(data=dfp6400, fun=pchisq, args=list(df=2),
								mapping=aes(x=x, color='chisq2'), linetype='solid') +
	xlim(c(0.0, 7.5)) +
	scale_linetype(name='n') +
	guides(color=guide_legend(title='Law'))
@
...
<<table, results='asis'>>=
library(xtable)
df <- data.frame(as.integer(n), D)
colnames(df) <- c("$n$", "$D_n$")
print(xtable(df, caption='Расстояние Колмогорова от
			 эмпирической функции распределения до предельного закона в зависимости
			 от объема выборки $n$.', digits=c(0,0,6)), sanitize.text.function=function(x){x},
...
<<fit>>=
fit <- lm(log(D) ~ log(n), data=df)
@

<<plotDn, fig.height=4, fig.cap=caption>>=
caption <- 'Данные зависимости расстояния Колмогорова $D$ от
объема выборки $n$ и построенный по этим данным степенной тренд.'

plot(n, D, type='p', ylim=c(0,0.1), lwd=1.5, axes=F)
lines(seq(1,6400,10), exp(predict(fit, newdata=data.frame(n=seq(1,6400,10)))),
			col='red')
points(n, exp(predict(fit)), col='red')
legend(4000, 0.1, c('data', 'trend', '(1426, 0.01)'),
			 col=c('black', 'red', 'blue'), pch=rep(21, 3),
			 lty=c(NA, 'solid', NULL))
lines(y=c(0.01, 0.01), x=c(0, 6400), lty='dashed')
lines(y=c(0, 0.1), x=c(1426, 1426), lty='dashed')
axis(2, seq(0, 0.1, 0.01))
axis(1, seq(0, 7000, 500))
points(1426, 0.01, col='blue')
@

Полученная модель:
\[
	D(n) = \Sexpr{fit$coefficients[1]}\ n^{\Sexpr{fit$coefficients[2]}}
\]

Коэффициент детерминации $R^2 = \Sexpr{summary(fit)$r.squared}$.

\subsection{Определение объема выборки $n$, начиная с которого расстояние до
предельного закона распределения не превышает заданного $\varepsilon$}

Необходимо решить уравнение $\varepsilon = a n^{-b}$ относительно $n$. Для
этого прологарифмируем обе части уравнения. Воспользуемся десятичным
логарифмом. Выразим $n$ из полученного уравнения, в итоге получим:

<<varepsilon>>=
eps <- 0.01
a <- fit$coefficients[1]
b <- fit$coefficients[2]
n <- round(exp(((log(eps) - a) / b)))
@

\[
	n(\varepsilon) = \exp\left\{\frac{\log (\varepsilon) - \log a}{b}\right\}.
\]

\[
	n(0.01) = \Sexpr{round(exp(((log(eps) - a) / b)))}
\]

Для проверки предсказания по модели проведем контрольный эксперимент.

\subsection{Контрольльный эксперимент}

<<ctrlmod>>=
n <- 1426
dfc <- statmod('JB', n, N, 'norm')
@

<<ctrld>>=
D <- ks.test(dfc$x, pchisq, df=2)$stat
@

В результате проведения контрольного эксперимента при $n=1426$ расстояине от 
эмпирической функции распределения до предельного закона составило
\[
	D_{1426} = \Sexpr{D}, 
\]
это значение находится в пределах
заданной статистической погрешности моделирования $\delta = 0.001$, т.е. $0.01
- \delta < \Sexpr{D} < 0.01 + \delta$.

\section{Заключение}

В результате проведения экспериментальных исследований были смоделированы
распределения статистики Жарка-Бера, результаты показали сходимость
распределения к предельному, была построена экспериментальная зависимость 
расстояния от эмпирического распределения до предельного закона в классе
степенных функций.

Результаты экспериментальных исследований хорошо согласуются с теоретическими
положениями, в частности с теоремой Колмогорова, а также с другими
