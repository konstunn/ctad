...
<<norm-vs-dexp-plot-function>>=
plot.dexp.vs.norm <- function(stat, stat.cap)
{
	df <- read.data(stat=stat, h='norm', n=10)
	dft <- read.data(stat=stat, h='dexp', n=10)
	df <- rbind(df, dft)

	cap <- sprintf('Эмпирическое распределение статистики %s при нормальном
		и двустороннем экспоненциальном законах.', stat.cap)

	assign('caption', cap, envir=.GlobalEnv)

	p <- ggplot() +
		stat_ecdf(mapping=aes(x=x, linetype=factor(h)), data=df) +
		scale_linetype('true H')

	print(p)
}
@

<<frosini-dexp-norm-plot, fig.cap=caption>>=
plot.dexp.vs.norm('frosini', 'Фросини')
@

<<t1-dexp-vs-norm-plot, fig.cap=caption>>=
plot.dexp.vs.norm('t1', 'Хегази-Грина $T_1$')
@

<<t2-dexp-vs-norm-plot, fig.cap=caption>>=
plot.dexp.vs.norm('t2', 'Хегази-Грина $T_2$')
@

<<giri-dexp-vs-norm-plot, fig.cap=caption>>=
plot.dexp.vs.norm('giri', 'Гири')
@

<<dhp-dexp-vs-norm-plot, fig.cap=caption>>=
plot.dexp.vs.norm('dhp', 'Дэвида-Хартли-Пирсона')
@

<<spiegel-dexp-vs-norm-plot, fig.cap=caption>>=
plot.dexp.vs.norm('spiegel', 'Шпигельхальтера')
@

<<royston-dexp-vs-norm-plot, fig.cap=caption>>=
plot.dexp.vs.norm('royston', 'Ройстона')
@
...
stats <- c('frosini', 't1', 't2', 'giri', 'spiegel', 'dhp', 'royston',
					 'eppspulley')
...
<<agostino-functions>>=
library(PoweR)
z1 <- function(x) statcompute(24, x)$statistic
z2 <- function(x) statcompute(6, x)$statistic
@

<<mod-data-for-power>>=
library(moments)

stats <- c('skewness', 'kurtosis', 'shapiro', 'z1', 'z2')

N <- 16600

for (stat in stats) {
	for (h in hs) {
		dft <- statmod(stat, n, N, h)
		dft$N <- NULL
		df <- rbind(df, dft)
	}
}
