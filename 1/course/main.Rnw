...
Проверим относительное количество городского населения в США (1973 год) на
нормальность.
...
\begin{itemize}
		\item Значение статистики $T_{EP} = \Sexpr{S}$
		\item Значение p-value: $\Sexpr{pval}$
		\item Количество повторений Монте-Карло $N = \Sexpr{N}$
\end{itemize}

<<usahist, fig.cap=caption, fig.height=4>>=
caption <- 'Частотная гистограмма относительного количества городского населения'
hist(X, 8)
@
...
относительно не высоком числе повторений Монте-Карло.
...
<<appendix1, echo=T, eval=F, tidy=T>>=
<<Tep>>
<<mypvalMC>>
<<TepMC>>
<<statmod-fun>>
