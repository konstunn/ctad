...
различных альтернативных гипотезах. Объем выборки $n = \Sexpr{n}$,
число испытаний Монте-Карло при вычислении p-value $N = \Sexpr{N}$.
...
S <- Tep(X)
pval <- mypvalMC(X, N, 2)
law <- sprintf('Обобщ. нормальный(4,0,1)')
...
<<pwrtable, results='asis', cache=FALSE, purl=F>>=
caption <- 'Результаты проверки нормальности с помощью PoweR.'

colnames(dfp) <- c('Закон распределения', 'Значение статистики',
									 'p-value')
print(xtable(dfp, digits=7, caption=caption), include.rownames=F,
			sanitize.text.function=function(x){x}, caption.placement='top')
@

<<readiswrez, cache=FALSE, purl=F>>=
# FIXME: don't repeat yourself
law <- sprintf('Нормальный(0,1)')
df <- read.table('data/norm200.rez')
S <- df[2,1]
pval <- df[1,1]
dfp <- data.frame(law=law, S=S, pval=pval)

law <- sprintf('Лапласа(0,1)')
df <- read.table('data/laplace200.rez')
S <- df[2,1]
pval <- df[1,1]
dfa <- data.frame(law=law, S=S, pval=pval)
dfp <- rbind(dfp, dfa)
...
law <- sprintf('Логистический(0,1)')
df <- read.table('data/logis200.rez')
S <- df[2,1]
pval <- df[1,1]
dfa <- data.frame(law=law, S=S, pval=pval)
dfp <- rbind(dfp, dfa)
...
law <- sprintf('Обобщ. нормальный(4,0,1)')
df <- read.table('data/pgnorm200.rez')
S <- df[2,1]
pval <- df[1,1]
dfa <- data.frame(law=law, S=S, pval=pval)
dfp <- rbind(dfp, dfa)

dfp$S <- as.character(dfp$S)
dfp$S <- as.numeric(dfp$S)
dfp$pval <- as.character(dfp$pval)
dfp$pval <- as.numeric(dfp$pval)
@

<<iswtable, results='asis', cache=FALSE, purl=F>>=
caption <- 'Результаты проверки нормальности с помощью ISW.'
...
<<pdfsplots, fig.cap=caption, cache=FALSE, purl=F>>=
...
stat_function(aes(x, colour='norm(0,1)'), data=data.frame(x=rnorm(n)),
								fun=dnorm) +
								 stat_function(aes(x, colour='pgnorm(4,0,1)'),
															 data=data.frame(x=rpgnorm(n, 4, 0, 1)),
							fun=function(x) dpgnorm(x, 4, 0, 1)) +
stat_function(aes(x, colour='logis(0,1)'), data=data.frame(x=rlogis(n)),
...
<<iswrezusa>>=
df <- read.table('data/usa.rez')
S <- df[2,1]
pval <- df[1,1]
@

Значение статистики: \Sexpr{S} \\
\indent p-value: \Sexpr{pval}

<<usahist, fig.cap=caption, fig.height=4, purl=F>>=
...
<<usadensity, fig.cap=caption, fig.height=4, purl=F>>=
...
распределения статистики с помощью R (PoweR) и ISW. Результаты хорошо
согласуются. В связи с этим предполагается считать, что соответствующие
алгоритмы реализованы корректно.

По результатам проверки нормальности с помощью ISW видно, что p-value
вычисляется по таблице квантилей эмпирического распределения статистики.
