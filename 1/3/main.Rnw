...
ggplot(data=dfpwrs, mapping=aes(x=alpha, y=power,
																linetype=factor(k), colour=factor(k))) +
...
ggplot(data=dfpwrc, mapping=aes(x=alpha, y=power,
																linetype=factor(k), colour=factor(k))) +
	geom_smooth(size=.5, se=F) +
	scale_x_log10() +
	scale_linetype('Number of intervals') +
	scale_colour_manual(name='Number of intervals', values=cbPalette)
@
...
При проверке сложной гипотезы мощность критерия ниже, чем при проверке простой. 
