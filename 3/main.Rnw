...
<<stat-mod-src, code=readLines('src/r.R')>>=
@
...
\newpage
	...
	При сложной гипотезе распределение статистики лежит между $\chi^2_{k-1}$ и
		$\chi^2_{k-m-1}$ при оценивании $m$ параметров методом максимального
		правдоподобие по негруппированным данным.
...
<<simple-gamma-ksfit-plot, fig.cap=caption>>=
attach(gamma.fit)
ggplot(dfs, aes(x, linetype='Empirical (simple H)')) +
	stat_ecdf() +
...
<<simple-gamma-ksfit-dplot, fig.cap=caption>>=
attach(gamma.fit)
ggplot(dfs, aes(x, linetype='Empirical (simple H)')) +
	geom_density() +
...
<<simple-gamma-cvmfit-plot, fig.cap=caption>>=
caption <- 'Эмпирическое распределение статистики и Гамма-распределение.'
attach(gamma.fit)
ggplot(dfs, aes(x, linetype='Empirical (simple H)')) +
	stat_ecdf() +
...
<<simple-gamma-cvmfit-dplot, fig.cap=caption>>=
attach(gamma.fit)
ggplot(dfs, aes(x, linetype='Empirical (simple H)')) +
	geom_density() +
...
<<complex-gamma-mlfit-plot, fig.cap=caption>>=
caption <- 'Эмпирическое распределение статистики и Гамма-распределение.'
attach(gamma.fit)
...
<<complex-gamma-mlfit-dplot, fig.cap=caption>>=
attach(gamma.fit)
ggplot(dfs, aes(x, linetype='Empirical (complex H)')) +
	geom_density() +
	stat_function(aes(x, linetype='Gamma, ML-fit'), fun=dgamma,
...
<<complex-gamma-ksfit-pplot, fig.cap=caption>>=
attach(gamma.fit)
ggplot(dfs, aes(x, linetype='Empirical (complex H)')) +
	stat_ecdf() +
...
<<complex-gamma-ksfit-dplot, fig.cap=caption>>=
ggplot(dfs, aes(x, linetype='Empirical (complex H)')) +
	geom_density() +
...
<<complex-gamma-cvmfit-plots, fig.cap=caption>>=
attach(gamma.fit)
ggplot(dfs, aes(x, linetype='Empirical (complex H)')) +
	stat_ecdf() +
...
\bibitem{lemeshko-mono} Статистический анализ данных, моделирование и
...
\bibitem{knitr1} Yihui Xie (2016). knitr: A General-Purpose Package for Dynamic
