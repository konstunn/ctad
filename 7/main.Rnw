...
{
	dfnull <- subset(dfin, h0=='pgnorm')
	dfalt <- subset(dfin, h0=='norm')
...
<<alpha-power-function-norm-cauchy>>=
# TODO: add for left sided (Shapiro) and two sided (Agostino)
power <- function(dfin, alpha, side='right')
{
	dfnull <- subset(dfin, h0=='norm')
	dfalt <- subset(dfin, h0=='cauchy')
...
df <- power(df, alpha, side='left')
@

<<shapiro-cauchy-norm-power-plot, fig.cap=caption>>=
caption <- 'Оперативная характеристика критерия Шапиро-Уилка.'
ggplot() +
...
caption <- 'Оперативная характеристика критерия Эппса-Палли.'
ggplot() +
	geom_line(data=df, mapping=aes(x=alpha, y=power))
@
