...
df <- read.table('data/h0aog5.dat', skip=2, col.names='x')
...
df <- read.table('data/h0aog9.dat', skip=2, col.names='x')
...
df <- read.table('data/h0aog15.dat', skip=2, col.names='x')
...
<<complex-den-plot, fig.height=3, fig.cap=caption, warning=FALSE>>=
df5c <- subset(dfc, k==5)
...
<<pears-simple-power>>=
kv <- 5
df <- chisq.statmod.gof2cauchy(100, 16600, 'simple', 'cauchy', kv)
dft <- chisq.statmod.gof2cauchy(100, 16600, 'simple', 'norm', kv)
df <- merge(df, dft, all=TRUE)
@

<<calc-power-nik-pears-simple, cache=FALSE>>=
dfpns <- power(subset(dfs, k==kv), alpha)
dfpps <- power(df, alpha)
@

<<power-plot-nik-pears-simple, fig.cap=caption, fig.height=3, warning=FALSE>>=
caption <- 'Оперативные характеристики критериев Пирсона и Никулина, k=5.'
ggplot() +
